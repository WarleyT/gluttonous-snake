...
export async function onRequest(context) {
  ...
  try {
    ...
    if (request.method === "POST") {
      ...
      if (env.DB) {
        await insertRecordIntoD1(env.DB, record);
        return json(await readLeaderboardsFromD1(env.DB), 201);
      }

      if (env.LEADERBOARD) {
        const boards = await readLeaderboardsFromKV(env.LEADERBOARD);
        boards[record.mode].push(record);
        const ranked = rankBoards(boards);
        await env.LEADERBOARD.put(SCORE_KEY, JSON.stringify(ranked));
        return json(ranked, 201);
      }

      throw new Error("Missing Cloudflare binding: DB or LEADERBOARD");
    }
...
async function readLeaderboards(env) {
  if (env.DB) return readLeaderboardsFromD1(env.DB);
  if (env.LEADERBOARD) return readLeaderboardsFromKV(env.LEADERBOARD);
  throw new Error("Missing Cloudflare binding: DB or LEADERBOARD");
}

async function readLeaderboardsFromD1(db) {
...
async function readLeaderboardsFromKV(store) {
  const raw = await store.get(SCORE_KEY);
...
async function insertRecordIntoD1(db, record) {
  await db
    .prepare(
